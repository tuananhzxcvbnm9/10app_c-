...
public sealed class ApiClient(HttpClient httpClient)
{
    ...
    {
        var response = await httpClient.GetFromJsonAsync<List<ListItemModel>>("metric-records", cancellationToken);
        return response ?? [];
    }

    public async Task<ListItemModel?> CreateAsync(string name, CancellationToken cancellationToken = default)
    {
        var response = await httpClient.PostAsync($"metric-records?name={Uri.EscapeDataString(name)}", null, cancellationToken);
