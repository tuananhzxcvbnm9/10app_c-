...
static async Task<IResult> GetAll(IOrderRepository repo, CancellationToken ct)
    => Results.Ok(await repo.GetAllAsync(ct));

static async Task<IResult> CreateItem(string name, IOrderRepository repo, HttpContext httpContext, CancellationToken ct)
{
    var item = await repo.AddAsync(new Ecommerce.Domain.Entities.Order { Name = name }, ct);
...
v1.MapGet("/orders", GetAll);
v1.MapPost("/orders", CreateItem);
legacy.MapGet("/orders", GetAll);
legacy.MapPost("/orders", CreateItem);
