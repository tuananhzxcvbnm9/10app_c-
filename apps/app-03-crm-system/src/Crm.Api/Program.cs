...
static async Task<IResult> GetAll(ILeadRepository repo, CancellationToken ct)
    => Results.Ok(await repo.GetAllAsync(ct));

static async Task<IResult> CreateItem(string name, ILeadRepository repo, HttpContext httpContext, CancellationToken ct)
{
    var item = await repo.AddAsync(new Crm.Domain.Entities.Lead { Name = name }, ct);
    var correlationId = httpContext.TraceIdentifier;
    httpContext.Response.Headers[CorrelationIdMiddleware.HeaderName] = correlationId;
    return Results.Created($"/leads/{item.Id}", item);
}

v1.MapGet("/leads", GetAll);
v1.MapPost("/leads", CreateItem);
legacy.MapGet("/leads", GetAll);
legacy.MapPost("/leads", CreateItem);
