...
using Expense.Infrastructure.Seed;

var builder = WebApplication.CreateBuilder(args);

builder.Host.UseSerilog((ctx, cfg) => cfg
    .ReadFrom.Configuration(ctx.Configuration)
    .Enrich.WithProperty("Application", "Expense.Api")
...
static async Task<IResult> GetAll(IExpenseRecordRepository repo, CancellationToken ct)
    => Results.Ok(await repo.GetAllAsync(ct));

static async Task<IResult> CreateItem(string name, IExpenseRecordRepository repo, HttpContext httpContext, CancellationToken ct)
{
    var item = await repo.AddAsync(new Expense.Domain.Entities.ExpenseRecord { Name = name }, ct);
    var correlationId = httpContext.TraceIdentifier;
    httpContext.Response.Headers[CorrelationIdMiddleware.HeaderName] = correlationId;
    return Results.Created($"/expense-records/{item.Id}", item);
}

v1.MapGet("/expense-records", GetAll);
v1.MapPost("/expense-records", CreateItem);
legacy.MapGet("/expense-records", GetAll);
legacy.MapPost("/expense-records", CreateItem);
