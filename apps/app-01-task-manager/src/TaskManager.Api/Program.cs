...
static async Task<IResult> GetAll(ITaskRepository repo, CancellationToken ct)
    => Results.Ok(await repo.GetAllAsync(ct));

static async Task<IResult> CreateItem(string name, ITaskRepository repo, HttpContext httpContext, CancellationToken ct)
{
    var item = await repo.AddAsync(new TaskManager.Domain.Entities.Task { Name = name }, ct);
    var correlationId = httpContext.TraceIdentifier;
    httpContext.Response.Headers[CorrelationIdMiddleware.HeaderName] = correlationId;
    return Results.Created($"/tasks/{item.Id}", item);
}

v1.MapGet("/tasks", GetAll);
v1.MapPost("/tasks", CreateItem);
legacy.MapGet("/tasks", GetAll);
legacy.MapPost("/tasks", CreateItem);
