...
using Booking.Infrastructure.Seed;
...
static async Task<IResult> GetAll(IBookingRepository repo, CancellationToken ct)
    => Results.Ok(await repo.GetAllAsync(ct));

static async Task<IResult> CreateItem(string name, IBookingRepository repo, HttpContext httpContext, CancellationToken ct)
{
    var item = await repo.AddAsync(new Booking.Domain.Entities.Booking { Name = name }, ct);
    var correlationId = httpContext.TraceIdentifier;
    httpContext.Response.Headers[CorrelationIdMiddleware.HeaderName] = correlationId;
    return Results.Created($"/bookings/{item.Id}", item);
}

v1.MapGet("/bookings", GetAll);
v1.MapPost("/bookings", CreateItem);
legacy.MapGet("/bookings", GetAll);
legacy.MapPost("/bookings", CreateItem);
